...
import { updateBusinessPrice, updateLoyaltyInterval, updateBusinessProfile } from '@/lib/actions';
import BusinessUrlCard from '@/components/BusinessUrlCard';
...
export default async function FinanceOverviewPage({
  ...
  const { data: business } = await supabase
    .from('businesses')
    .select('default_price, commission_rate, loyalty_interval, paystack_subaccount_code, address, bio, slug')
    .eq('id', businessId)
    .single();
  ...
  const { data: monthTransactions } = await supabase
    .from('transactions')
    .select('amount, platform_fee, used_subaccount')
  ...
  return (
    <div className="space-y-6">
      ...
      {business?.slug && <BusinessUrlCard slug={business.slug} />}

      <div className="card max-w-lg">
        <h2 className="font-semibold mb-1">Business profile</h2>
        <p className="text-xs text-neutral-500 mb-3">
          Shown on your public BarbFlow page so customers can find and learn about your shop.
        </p>
        <form action={updateBusinessProfile} className="space-y-3">
          <div>
            <label className="text-xs text-neutral-500 block mb-1">Address / location</label>
            <input
              name="address"
              type="text"
          ...
          <div>
            <label className="text-xs text-neutral-500 block mb-1">Short bio / description</label>
            <textarea
              name="bio"
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {(reportTransactions ?? []).map((t) => {
                const netToBank = Number(t.business_amount);
                return (
